...
from ele.Spell import AtkSpell
...
class Game(State):
    ...
    def basic_atk(self):
        if Root.turn:
            ...
            self.get_mp(3)
            self.show_actions(f'{self.player1.name}(이)가 기본 공격을 했다.', f'{self.player2.name}(은)는 {self.player1.strength}의 데미지를 입었다.')
        else:
            self.player2.hit(self.player1)
            Game.death(self.player1, self.player2)
            self.get_mp(3)
            self.show_actions(f'{self.player2.name}(이)가 기본 공격을 했다.', f'{self.player1.name}(은)는 {self.player2.strength}의 데미지를 입었다.')
